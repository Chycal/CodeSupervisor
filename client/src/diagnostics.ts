...
export class DiagnosticsHandler {
  ...
  public updateDiagnostics(
    ...
    for (const issue of issues) {
      ...
      diagnostic.source = "CodeSupervisior";
    ...
    if (diagnostics.length > 0) {
      vscode.window
        .showInformationMessage(
          `CodeSupervisior: Found ${diagnostics.length} security ${
  ...
  public getQuickFixes(diagnostic: vscode.Diagnostic): vscode.CodeAction[] {
    ...
    fix.command = {
      command: "CodeSupervisior.quickFix",
...
export class DiagnosticProvider implements vscode.CodeActionProvider {
  ...
  provideCodeActions(
    ...
    for (const diagnostic of context.diagnostics) {
      if (diagnostic.source === "CodeSupervisior") {
